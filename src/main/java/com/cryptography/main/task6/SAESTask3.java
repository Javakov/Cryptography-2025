...
/**
 * Задание 6.3 (S-AES, CBC):
 * Расшифровать файл dd5_saes_cbc_c_all.bmp – зашифрованное шифром S_AES изображение в формате bmp.
 * Матрица для преобразования MixColumns: [['a', 'c'], ['8', '6']].
 * Неприводимый многочлен: x⁴+x³+1 (изначально указано было x⁴+x+1, но оказалось неверно).
 * Режим шифрования CBC.
 * Ключ равен 1021.
 * Вектор инициализации равен 456.
 ...
 * (сигнатура "BM" не совпадала). Был создан тестовый класс SAESTask3Test, который систематически
 * перебирал различные комбинации параметров:
 ...
 * - Матрица MixColumns: {{0x0A, 0x0C}, {0x08, 0x06}} - как в исходном задании, верно
 * - Полином: 0b11001 = x⁴+x³+1 (а не x⁴+x+1 как было указано изначально)
 * - Порядок байт: (hi << 8) | lo - как в SAESTask1
 * 
 * После исправления полинома на x⁴+x³+1 расшифровка дала корректный BMP файл с сигнатурой "BM" (0x42 0x4D).
 */
public class SAESTask3 {

    private static final String INPUT = "6/in/dd5_saes_cbc_c_all.bmp";
    private static final String OUT_DECRYPT = "6/out/dd5_decrypted.bmp";
    private static final String OUT_REENCRYPT = "6/out/dd5_reencrypted_cbc.bmp";

    private static final int HEADER_KEEP = 50;
    private static final int KEY_DECIMAL = 1021;
    private static final int IV_DECIMAL = 456;

    // Матрица из условия: [['a','c'],['8','6']] => [[10,12],[8,6]] - ПРАВИЛЬНО
    private static final int[][] MIX = {{0x0A, 0x0C}, {0x08, 0x06}};
    // Полином: ИЗНАЧАЛЬНО было указано x⁴+x+1 (0b10011) - НЕВЕРНО
    // ИСПРАВЛЕНО на x⁴+x³+1 (0b11001) - найдено подбором параметров
    private static final int MOD = 0b11001; // x^4 + x^3 + 1

    public static void main(String[] args) throws Exception {
        byte[] enc = FileUtils.readResource(INPUT);
        System.out.println("Задание 6.3 (S-AES, CBC)\nВход: " + INPUT);
        ...
        byte[] dec = cbcDecrypt(enc, ks, IV_DECIMAL & 0xFFFF);
        ...
        if (reenc.length > HEADER_KEEP) {
            ...
            byte[] bodyEnc = cbcEncrypt(body, ks, IV_DECIMAL & 0xFFFF);
     ...
     * plaintext_i = decrypt(ciphertext_i) XOR ciphertext_{i-1}, где ciphertext_{-1} = IV
     * Работает с 16-битными блоками (2 байта) в little-endian порядке
     */
    private static byte[] cbcDecrypt(byte[] data, int[] ks, int iv16) {
        SAESCipher c = new SAESCipher(MIX, MOD);
        int k0 = ks[0], k1 = ks[1], k2 = ks[2];
        byte[] out = new byte[data.length];
        int prevCipher = iv16 & 0xFFFF; // предыдущий зашифрованный блок (для первого блока = IV)
        int idx = 0;
        while (idx + 1 < data.length) {
            // Читаем зашифрованный блок (2 байта, как в SPN1: lo | (hi << 8))
            int lo = data[idx] & 0xFF;
            int hi = data[idx + 1] & 0xFF;
            int cipherBlock16 = (hi << 8) | lo;  // как в SAESTask1
            
            // Расшифровываем блок
            int decrypted16 = c.decrypt(cipherBlock16, k0, k1, k2) & 0xFFFF;
            
            // XOR с предыдущим зашифрованным блоком (или IV для первого блока)
            int plainBlock16 = decrypted16 ^ prevCipher;
            
            // Записываем результат в том же порядке
            out[idx] = (byte) (plainBlock16 & 0xFF);  // младший байт
            out[idx + 1] = (byte) ((plainBlock16 >> 8) & 0xFF);  // старший байт
            
            // Сохраняем текущий зашифрованный блок для следующей итерации
            prevCipher = cipherBlock16;
            idx += 2;
        }
     ...
     * ciphertext_i = encrypt(plaintext_i XOR ciphertext_{i-1}), где ciphertext_{-1} = IV
     * Работает с 16-битными блоками (2 байта) в little-endian порядке
     */
    private static byte[] cbcEncrypt(byte[] data, int[] ks, int iv16) {
        SAESCipher c = new SAESCipher(MIX, MOD);
        int k0 = ks[0], k1 = ks[1], k2 = ks[2];
        byte[] out = new byte[data.length];
        int prevCipher = iv16 & 0xFFFF; // предыдущий зашифрованный блок (для первого блока = IV)
        int idx = 0;
        while (idx + 1 < data.length) {
            // Читаем открытый блок (2 байта, как в SPN1: lo | (hi << 8))
            int lo = data[idx] & 0xFF;
            int hi = data[idx + 1] & 0xFF;
            int plainBlock16 = (hi << 8) | lo;  // как в SAESTask1
            
            // XOR с предыдущим зашифрованным блоком (или IV для первого блока)
            int xorBlock16 = plainBlock16 ^ prevCipher;
            
            // Шифруем результат XOR
            int cipherBlock16 = c.encrypt(xorBlock16, k0, k1, k2) & 0xFFFF;
            
            // Записываем результат в том же порядке
            out[idx] = (byte) (cipherBlock16 & 0xFF);  // младший байт
            out[idx + 1] = (byte) ((cipherBlock16 >> 8) & 0xFF);  // старший байт
